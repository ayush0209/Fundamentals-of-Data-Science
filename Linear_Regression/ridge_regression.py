...
for t in range(len(regularization_coefficient)):
	...
	while abs(prev_Loss-Loss)>precision:
		...
		for i in range(len(partial_derivative)):
			...
			partial_derivative[i]=((2*temp)/len(row_train_rem))+(2*regularization_coefficient[t]*W[i])
	...
	L1_loss_list.append(loss2)

min_loss_index=0
for t in range(len(regularization_coefficient)):
	if(L1_loss_list[t]<L1_loss_list[min_loss_index]):
		min_loss_index=t
...
plt.plot(regularization_coefficient,L1_loss_list)
